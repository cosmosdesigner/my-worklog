...
fn insight_reports_group_duplicates_and_status_suppresses_noise() {
    ...
    fs::write(
        ...
        format!(
            ...
            timestamp(yesterday, 9, 6),
            timestamp(yesterday, 9, 7),
            timestamp(yesterday, 9, 8)
        ),
    )
    .expect("write fixture");
    let db = WorklogDb::open(&dir.path().join("worklog.sqlite")).expect("open db");
...
fn compact_done_and_status_skip_meta_dumps_and_truncate_items() {
    ...
    fs::write(
        ...
        format!(
            ...
            timestamp(yesterday, 1, 2),
            timestamp(yesterday, 1, 3),
            timestamp(yesterday, 1, 4),
            timestamp(yesterday, 1, 5),
            timestamp(yesterday, 1, 6)
        ),
    )
    .expect("write fixture");
    let db = WorklogDb::open(&dir.path().join("worklog.sqlite")).expect("open db");
...
#[test]
fn compact_status_uses_strict_semantic_predicates() {
    let dir = tempdir().expect("tempdir");
    let spool = dir.path().join("spool/opencode");
    fs::create_dir_all(&spool).expect("create spool");
    let yesterday = Local::now().date_naive() - Days::new(1);
    fs::write(
        spool.join("events.jsonl"),
        format!(
            "{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"decision\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"Decision: keep compact insights deterministic and local-only\"}}\n{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"blocker\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"Blocker: adapter install test missing\"}}\n{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"todo\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"TODO: follow up on compact predicate regression\"}}\n{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"summary-use\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"Adapter tests are green now. I am wiring import to use an explicit spool path\"}}\n{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"summary-error\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"The diagnostics table includes an error column for context\"}}\n{{\"source_agent\":\"opencode\",\"source_session_id\":\"s5\",\"source_event_id\":\"summary-next\",\"type\":\"assistant_message\",\"role\":\"assistant\",\"timestamp\":\"{}\",\"content\":\"The next section describes CLI behavior\"}}\n",
            timestamp(yesterday, 1, 0),
            timestamp(yesterday, 1, 1),
            timestamp(yesterday, 1, 2),
            timestamp(yesterday, 1, 3),
    ...
    assert!(status.contains("Decision: keep compact insights deterministic and local-only"));
    assert!(status.contains("Blocker: adapter install test missing"));
    assert!(status.contains("TODO: follow up on compact predicate regression"));
    assert!(!status.contains("Adapter tests are green now"));
    assert!(!status.contains("error column"));
    assert!(!status.contains("next section"));
}
